                ...
                if success:
                    ...
                    example = tf.train.Example(
                        features=tf.train.Features(
                            feature={
                                'raw_data':
                                _bytes_feature(raw_data.tostring()),
                                'features':
                                _bytes_feature(raw_data_array.tostring()),
                                'fname':
                                _bytes_feature(str.encode(file_n))
                            }))
...
def run_list(dirs, output_folder):
    ...
    if output_folder is None:
        output_folder = os.path.abspath(os.path.join(root_folder,
                                                     os.pardir)) + os.sep+'raw'
...
def extract_file(input_file):
    try:
        (raw_data, raw_label, raw_start, raw_length) = labelop.get_label_raw(
            input_file, FLAGS.basecall_group, FLAGS.basecall_subgroup)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        help=
        "Directory that store the fast5 files, multiple directories separted by commas."
    )
    parser.add_argument('-o', '--output', required=True, help="Output folder to store tfrecords for training/validation")
    parser.add_argument(
        '--basecall_group',
        default="RawGenomeCorrected_000",
        help=
        'The group in fast5 to extract the training data from. e.g. RawGenomeCorrected_000'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=
        'Subgroup for resquiggled data in fast5. Default is BaseCalled_template'
    )
    parser.add_argument(
        '--mode',
        default='dna',
        help=
        'Type of data to basecall, default is dna'
    )
