...
class DataSet(object):
    ...
    def read_into_memory(self, index):
        ...
        if not self.for_eval:
            label = np.asarray(
                list(
                    zip([self._label[i] for i in index],
            ...
            # Shuffle the data
                ...
                if event_rest_part.shape[0] == 0:
                    ...
                    event_batch = np.concatenate(
                        (event_rest_part, event_new_part), axis=0)
...
def read_data_for_eval(file_path,
    ...
    if not file_path.endswith('.signal'):
        raise ValueError('please run raw.py to extract signal from fast5 file.')
    else:
        event = list()
        event_len = list()
        label = list()
...
def read_cache_dataset(h5py_file_path):
    ...
    event_length = biglist(
        data_handle=event_length_h, length=event_len, cache=True)
    label = biglist(data_handle=label_h, length=label_len, cache=True)
    label_length = biglist(
        data_handle=label_length_h, length=label_len, cache=True)
    return DataSet(
        event=event,
...
def read_tfrecord(data_dir,
    ...
    with h5py.File(h5py_file_path, "a") as hdf5_record:
        event_h = hdf5_record.create_dataset(
            'event/record',
            dtype='float32',
            shape=(0, seq_length),
            maxshape=(None, seq_length))
        event_length_h = hdf5_record.create_dataset(
            'event/length',
            dtype='int32',
            shape=(0, ),
            maxshape=(None, ),
            chunks=True)
        label_h = hdf5_record.create_dataset(
            'label/record',
            dtype='int32',
            shape=(0, 0),
            maxshape=(None, seq_length))
        label_length_h = hdf5_record.create_dataset(
            'label/length', dtype='int32', shape=(0, ), maxshape=(None, ))
        event = biglist(data_handle=event_h, max_len=FLAGS.MAXLEN)
        event_length = biglist(
            data_handle=event_length_h, max_len=FLAGS.MAXLEN)
        label = biglist(data_handle=label_h, max_len=FLAGS.MAXLEN)
        label_length = biglist(
            data_handle=label_length_h, max_len=FLAGS.MAXLEN)
        count = 0
        file_count = 0
        ...
        for string_record in record_iterator:
            ...
            raw_data_string = (
                example.features.feature['raw_data'].bytes_list.value[0])
            ...
            group_size = 3
            features_data = [
                features_data[n:n + group_size]
                for n in range(0, len(features_data), group_size)
            ]
            f_signal = read_signal_tfrecord(raw_data)

            if len(f_signal) == 0:
                continue
            f_label = read_label_tfrecord(
                features_data, skip_start=skip_start, window_n=(k_mer - 1) / 2)
    ...
    with h5py.File(h5py_file_path, "a") as hdf5_record:
        ...
        for name in os.listdir(data_dir):
            if name.endswith(".signal"):
                ...
                try:
                    f_label = read_label(
                        data_dir + file_pre + '.label',
                        skip_start=10,
                        window_n=int((k_mer - 1) / 2))
                except:
                    sys.stdout.write(
                        "Read the label %s fail.Skipped." % (name))
                    continue
...
def read_raw(raw_signal, raw_label, max_seq_length):
    ...
    for indx, segment_length in enumerate(raw_label.length):
        ...
        if current_length + segment_length < max_seq_length:
            current_event += raw_signal[current_start:current_start +
                                        segment_length]
            current_label.append(current_base)
            current_length += segment_length
        else:
            # Save current event and label, conduct a quality controle step of the label.
            if current_length > (max_seq_length * MIN_SIGNAL_PRO
                                 ) and len(current_label) > MIN_LABEL_LENGTH:
                padding(
                    current_event, max_seq_length,
                    raw_signal[current_start + segment_length:current_start +
                               segment_length + max_seq_length])
...
def base2ind(base, alphabet_n=4, base_n=1):
    ...
    if alphabet_n == 4:
        ...
        raise ValueError('Alphabet number should be 4.')
    if base.isdigit():
        return int(base) / 256
    if ord(base) < 97:
